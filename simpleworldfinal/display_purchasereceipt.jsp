...
<%
	...
	{			
		...
		else
		{
			...
			z2="select display from permission where ur_id=\'"+t+"\' and page_name='purchasereceipt' ";
			...
			{
				String url = new String("http://localhost:8080/simpleworld/display_purchasereceipt.jsp?value="+p1);
			...
			z2="select update_form from permission where ur_id=\'"+t+"\' and page_name='purchasereceipt' ";
			rs2=st2.executeQuery(z2);
			String flag_update="";
			while(rs2.next())
			{
			...
			z2="select delete_form from permission where ur_id=\'"+t+"\' and page_name='purchasereceipt' ";
			rs2=st2.executeQuery(z2);
			String flag_delete="";
			while(rs2.next())
			{
		...
		<div id="page">
			<div id="content">
				<div class="post">
					...
					<h4>
						<% if(flag_form.equals("true")){%>
						<%
							z2="select state from purchase_receipt where pr_id=\'"+p1+"\'";
							rs2=st2.executeQuery(z2);
							if(rs2.next())
...
<%		QueryString1 = "SELECT state from purchase_receipt where pr_id=\'"+p1+"\'";
		...
		if(c.trim().equals("waiting"))
		{
			%>
		<tr>
		<td>&nbsp;<% if(flag_update.equals("true")){%><input type="button" value="Update" style="padding: 3px;" onclick="return validate();"></td><%
		}}%>
