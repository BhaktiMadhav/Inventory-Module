...
<%
	...
	{		
		...
		else
		{
			...
			{
				String url = new String("http://localhost:8080/simpleworld/update_purchasereceipt.jsp");
...
<%
	...
	String p6=request.getParameter("pr_id");
	String pr_id=p6.trim();
	...
	String v_type="Purchase Receipt";
		...
		out.println(cost);
		String sql;
		sql = "UPDATE purcahse_receipt SET v_id=?,address=?,mobile_no=?,item_code=?,quantity=?,total_amount=?,system_delete=?,state=? WHERE pr_id=?";

		ps = con2.prepareStatement(sql);
		st2=con2.createStatement();

		ps.setString(9,pr_id);
		ps.setString(1,v_id);
