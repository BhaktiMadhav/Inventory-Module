...
<%
	...
	{	
		...
		else
		{
			...
			{
				String url = new String("http://localhost:8080/simpleworld/user_action.jsp");
...
<%
				...
				String p3=request.getParameter("salary");
				String salary=p3.trim();

				String p4=request.getParameter("password");
				String password=p4.trim();
				
				String p5=request.getParameter("user_role");
				String user_role=p5.trim();
				...
				String ur_id="";
				String sql = "SELECT ur_id from user_role where user_role=\'"+user_role+"\'";
				rs2 = st2.executeQuery(sql);
				while (rs2.next())
				{
					ur_id=rs2.getString(1);
				}
				
				sql = "INSERT INTO user(name,address,salary,password,system_delete,ur_id) VALUES (?,?,?,?,?,?)";
				...
				ps.setString(5,sys_del);
				ps.setString(6,ur_id);
