...
try
	{				
		...
		else
		{
			...
			z2="select display from permission where ur_id=\'"+t+"\' and page_name='vendor' ";
			...
			{
				String url = new String("http://localhost:8080/simpleworld/vendor_by_region_graph.jsp");
				session.setAttribute("last_page",url);
			
			%>
<head>
	<title>VENDORS BY REGION </title>
	...
	<%
					...
					String QueryString = "SELECT COUNT(*) from vendor where region='Asia'";
					...
					QueryString = "SELECT COUNT(*) from vendor where region='Africa'";
					rs2= st2.executeQuery(QueryString);
					...
					QueryString = "SELECT COUNT(*) from vendor where region='America'";
					rs2= st2.executeQuery(QueryString);
					...
					QueryString = "SELECT COUNT(*) from vendor where region='Antartica'";
					rs2= st2.executeQuery(QueryString);
					...
					QueryString = "SELECT COUNT(*) from vendor where region='Europe'";
					rs2= st2.executeQuery(QueryString);
					...
					QueryString = "SELECT COUNT(*) from vendor where region='Australia'";
					rs2= st2.executeQuery(QueryString);
		...
		var f= <%=count6%>;
		  ...
		  window.onload = function () {
			  ...
			  bluffGraph.title = 'Graph of Vendors by Regions';
