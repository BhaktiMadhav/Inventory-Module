...
<%
		...
		z2="select list from permission where ur_id=\'"+t+"\' and page_name='purchasereceipt'";
        rs2=st2.executeQuery(z2);
        String flag_list="";
        while(rs2.next())
        {
			...
			<div id="content">
				<div class="post">
					...
					<div style="padding: 3px; background-color: rgb(226, 222, 203); border-radius: 3px 3px 3px 3px;">
						<% if(flag_form.equals("true")){%><a href="purchasereceipt_form.jsp"><input type="button"  value="+ New PurchaseReceipt" style="padding: 3px;"></a><%}%>
						&nbsp;&nbsp;
				...
				<%if(flag_list.equals("true"))
				{
					...
					String QueryString = "SELECT pr_id, state from purchase_receipt";
				 ...
				 <div class="post">
					<p><a href="display_purchasereceipt.jsp?value=<%=rs2.getString(1)%>"><%=rs2.getString(1)%></a> &nbsp; <%=rs2.getString(2)%></p>
				</div>
				<% 	
				}
				String url = new String("http://localhost:8080/simpleworld/purchasereceipt.jsp");
				session.setAttribute("last_page",url);
				}
				else
				{
					%><h1><%out.println(flag_list);%></h1><%
