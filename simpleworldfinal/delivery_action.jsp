...
<%
	...
	{	
		...
		else
		{
			...
			{
				String url = new String("http://localhost:8080/simpleworld/delivery_action.jsp");
...
<%
	...
	String v_type="Delivery Note";
		...
		String QueryString = "SELECT minimum_order_qty from item where item_code=\'"+itemcode+"\'";
		rs2 = st2.executeQuery(QueryString);
		
		if(rs2.next())
			qty1=Integer.parseInt(rs2.getString(1));
		
		int total=0;
		QueryString = "SELECT SUM(transaction_qty)from stock_ledger where item_code=\'"+itemcode+"\'";
		rs2 = st2.executeQuery(QueryString);
		...
		if((total-qty)>0)
		{
			...
			{
				int total1=0;
				total1=qty1-(total-qty);
				out.println("For Item "+itemcode+" :");%><br><%
				out.println("\nMimimum Order Quantity = "+qty1);%><br><%
				out.println("\nRemaining Quantity in Stock = "+(total-qty));%><br><%
				out.println("\nTotal Stock lesser than Minimum order Quantity = "+total1);%><br><%
			}
			
			QueryString = "SELECT cost from item where item_code=\'"+itemcode+"\'";
			rs2 = st2.executeQuery(QueryString);
			...
			String sql = "INSERT INTO delivery_note(c_id,shipping_address,mobile_no,item_code,quantity,total_amount,system_delete,state) VALUES (?,?,?,?,?,?,?,?)";
		...
		{
			out.println("The Stock for Item "+itemcode+ " is less than the quantity to be delivered.");
			out.println("Hence,Delivery Note failed !");
		}
		ps.close();
	}
		rs2.close();
